...
// Construit les 7 derniers jours (commandes + revenus) à partir des vraies commandes
function buildWeeklyData(orders: Order[]) {
  const days: { name: string; commandes: number; revenus: number; key: string }[] = [];
  const today = new Date();
  for (let i = 6; i >= 0; i--) {
    const d = new Date(today);
    d.setDate(today.getDate() - i);
    days.push({ name: DAY_LABELS[d.getDay()], commandes: 0, revenus: 0, key: d.toDateString() });
  }
...
export function Dashboard() {
  ...
  const loading = prodLoading || ordersLoading || custLoading;

  const weeklyData = useMemo(() => buildWeeklyData(orders), [orders]);
  const totalRevenue = useMemo(() => weeklyData.reduce((s, d) => s + d.revenus, 0), [weeklyData]);
  ...
  const topProducts = products.filter(p => p.is_featured).slice(0, 5);

  const stats = [
    { title: 'Total Produits', value: products.filter(p => p.is_active).length, icon: Package, iconColor: 'text-blue-600', iconBg: 'bg-blue-50', trend: `${products.length}`, trendUp: true, trendLabel: 'au total' },
    { title: 'Total Commandes', value: orders.length, icon: ShoppingBag, iconColor: 'text-primary', iconBg: 'bg-orange-50', trend: `+${orders.length}`, trendUp: true, trendLabel: 'cumulées' },
    { title: 'Commandes du jour', value: todayOrders, icon: TrendingUp, iconColor: 'text-emerald-600', iconBg: 'bg-emerald-50', trend: todayOrders > 0 ? `+${todayOrders}` : '0', trendUp: todayOrders > 0, trendLabel: "aujourd'hui" },
    { title: 'Clients', value: customers.length, icon: Users, iconColor: 'text-violet-600', iconBg: 'bg-violet-50', trend: `${customers.length}`, trendUp: true, trendLabel: 'enregistrés' },
    { title: 'Revenus (sem.)', value: formatPrice(totalRevenue), icon: Wallet, iconColor: 'text-amber-600', iconBg: 'bg-amber-50', trend: '7j', trendUp: true, trendLabel: '7 derniers jours', wide: true },
  ];
          ...
          <CardContent className="space-y-3">
            {loading
              ...
              : topProducts.length === 0
                ? <p className="text-sm text-muted-foreground py-4 text-center">Aucun produit vedette</p>
                  ...
                  <tbody>
                    {recentOrders.map((order) => (
                      <tr key={order.id} className="border-b border-gray-50 last:border-0">
                        <td className="py-3 text-sm font-medium text-[#303841]">{order.order_number}</td>
                        <td className="py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm truncate max-w-[100px]">{order.customer_name}</span>
                          </div>
                        </td>
